...
select "USER".id, pkg.id as credit_package_id,  pkg.credit_amount as purchased_credits, pkg.price as price_paid from "USER"
join "CREDIT_PACKAGE" pkg
on ("USER".name = '王小明' and pkg.name = '14 堂組合包方案')
or ("USER".name = '王小明' and pkg.name = '21 堂組合包方案')
or ("USER".name = '好野人' and pkg.name = '14 堂組合包方案');
...
select user_id, count(*) as total from "COURSE_BOOKING" where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and cancelled_at IS null
group by user_id;
...
select user_id, SUM(purchased_credits)-(select count(*) from "COURSE_BOOKING" where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') AND cancelled_at IS null ) AS remaining_credit from "CREDIT_PURCHASE" 
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
group by user_id;
-- ████████  █████   █     ███  
--   █ █   ██    █  █     █     
...
select  "USER".name AS 教練名稱 , "SKILL".name as 經驗年數, "COACH".experience_years as 專長名稱 from "COACH_LINK_SKILL" 
inner join "SKILL" on "SKILL".id = "COACH_LINK_SKILL".skill_id 
inner join "USER" on "USER".id = (select user_id from "COACH" where id = "COACH_LINK_SKILL".coach_id)
...
select  "CREDIT_PACKAGE".name as 組合包方案名稱, count(*) as 銷售數量 from "CREDIT_PURCHASE"
